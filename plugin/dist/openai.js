...
async function optimizeSelectedCode(selectedCode2) {
  const homePath = import_path.default.resolve(process.env.HOME || process.env.USERPROFILE || "", ".openai_api_key");
  const apiKey = import_fs.default.readFileSync(homePath, "utf8").trim();
  ...
  try {
    const response = await axios_default.post(apiUrl, jsonPayload, { headers });
...
var selectedCode = import_fs.default.readFileSync("/tmp/vim-openai-codeImprove-tmp.txt", { encoding: "utf8", flag: "r" });
if (selectedCode) {
